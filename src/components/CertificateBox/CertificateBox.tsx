...
export function CertificateBox(props : Certificate) {
  ...
  const [data,setData] = useState<LoadData>({name:'',subject:'',score:'',category:'',date:'',other:''});
  ...
  const loadAuth = async () => { 
    if(props.address){
      ...
      setAuth({address: props.address, name : authority[0],website : authority[1], image : authority[2],status : authority[3]})
    }
  }  
  ...
  <div className="card card-round ">
    <div className="row pinside60" style={{border:'3px solid #004F98',margin:'1px'}}>
      <div className="col-sm-12 text-center">
        <img  width="80px" src="./images/Badge.jpg" alt="white-logo" /><br/>
        <h2 className="text-blue text-weiight-bold">Cerficate of {props.category ? props.category : data?.category}</h2>
        ...
        <h5 className="mt30 font-weight-bold text-dark mb20">
          For Extraordinary performance in {props.subject ? props.subject : data?.subject}  on {props.date ? props.date : data.date}
        </h5>
        ...
        {
          (!auth.status)? <div className=" mt40 alert-danger text-left" >
