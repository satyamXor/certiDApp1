...
export class ViewCertificate1 extends Component<MyViewProperties,State> {
  ...
  async componentDidMount() {
    ...
    try{
      ...
      this.setState({auth : {address: txn[1], name : authority[0],website : authority[1], image : authority[2],status : authority[3]}})
                ...
                <div className="card card-round ">
                  <div className="row pinside60" style={{border:'3px solid #004F98',margin:'1px'}}>
                    <div className="col-sm-12 text-center">
                      <img  width="80px" src={process.env.PUBLIC_URL + "/images/Badge.jpg"}  alt="white-logo" /><br/>
                      <h2 className="text-blue text-weiight-bold">Cerficate of {this.state.Data?.category}</h2>
                      ...
                      <h5 className="font-weight-bold text-dark">
                       on {this.state.Data?.date}
                      </h5>
                      <div dangerouslySetInnerHTML={{ __html: this.state.Data?.other }} />

                      {
                          (!this.state.auth.status)? <div className=" mt40 alert-danger text-left" >
                          <Table striped bordered hover size="sm">
                                ...
                                <div className='text-danger small text-right'>*this signer is not listed on CertiDapp</div>
                         </div> : 
                         <div className={` alert alert-${this.color[this.state.auth.status]} mt40  text-left`}>
                          <Row>
                            <Col xs={6} md={3} >
                              <div className="whitebox2">
                                <img className="img-fluid" src="https://ipfs.eraswap.cloud/ipfs/QmdQALCX4i6bqao6onaK1WJ3RfJDgKUd1RdAqZc7HfgHZV" alt="white-logo" />
                              </div>
                            </Col>
                            <Col xs={12} md={9}>
                              <Table striped bordered hover size="sm">
                                <tbody>
                                  <tr>
                                    <th className={`text-${this.color[this.state.auth.status]}`}>Signer :</th>
                                    <td>{this.state.auth?.name}</td>
                                  </tr>
                                  <tr>
                                    <th className={`text-${this.color[this.state.auth.status]}`}>Signature:</th>
                              ...
                              <hr/>
                              <div className="text-right m-0 p-0 small">*This signer has listed with us & have valid KYC </div>
                            </Col>
                          </Row>
                         </div>
                        ...
                        <div className="col-md-12 text-center">
                          <div className="qr">
                              <img className="img-fluid" src="https://raw.githubusercontent.com/zpao/qrcode.react/HEAD/qrcode.png" alt="white-logo" />
                          </div> 
                        </div>
