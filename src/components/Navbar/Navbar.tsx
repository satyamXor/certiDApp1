...
const loadMetamask = async () => {
  ...
  function Wallet(){
    ...
    function Account(){
     return (<div style={{width:'400px', marginTop:'10px'}}>
        <p>ACTIVE ACCOUNT</p>
        <div className="dropdown-divider "></div> 
        
        {copied?(<i className="fa fa-check-circle" aria-hidden="true"></i>):(<i className="fa fa-clone" 
           style={{ marginLeft: 8, float: 'right', fontSize: '1.5em' }}
           onClick={()=>{copy(Address);setCopied(true);Swal.fire({
            position: 'bottom-end',
            title: 'Copied',
            showConfirmButton: false,
            timer: 1000
          })}}></i>)}
        <p><i className="fa fa-dot-circle-o text-success" ></i> You are Connected with : {Address}</p>
        <div className="dropdown-divider "></div>
        <button type="button" className="btn btn-outline-danger btn-block"
                onClick={()=>{window.wallet = undefined;setAddress(null);window.location.reload()}}>
          DisConnect</button>
            ...
            <Nav className="navbar-nav ml-auto">
              ...
              <NavDropdown
                ...
                className="btn btn-primary btn-sm js-scroll-trigger combtn" >
                    <NavDropdown.Item ><Link to="/RegisterCertificate">Register Certificate</Link></NavDropdown.Item>
                    <NavDropdown.Item ><Link to="/ViewCertificate">View  Certificate</Link></NavDropdown.Item>
                    <NavDropdown.Item ><Link to="/ListAuthorities">List  Authorities</Link></NavDropdown.Item>
                    <NavDropdown.Item ><Link to="/SignCertificate">Sign   Certificate</Link></NavDropdown.Item>
              </NavDropdown>
              <Nav.Link href="https://certidapp.com/" target="_blank">Blockchain Survey</Nav.Link>
         
              <Nav>
              <div className="dropdown nav mr10  ">
                  <button className=" btn btn-primary btn-xl js-scroll-trigger combtn con-wabtn" type="button" id="dropdownMenu" data-toggle="dropdown" >
                     {(!Address)?('Connect to Wallet'):(<><i className="fa fa-circle text-success"></i>&nbsp;{Address}</>) }
                  </button>
                  <div className="dropdown-menu px-2 dropdown-menu-right " aria-labelledby="dropdownMenu">
