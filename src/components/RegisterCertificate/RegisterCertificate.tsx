...
export class RegisterCertificate extends Component<State> {
  ...
  handleRegister = async (e) => {
    ...
    if(window.wallet){
      ...
      const sur = await window.certificateInstance.connect(window.wallet)
        ...
        try {
          const A = await window.certificateInstance.connect(x).estimateGas.registerCertificates(Args[0],Args[1],Args[2]);
          console.log(A);
        } catch (e) {
          ...
          <div className="rows text-center mt80 mb30">
            ...
            <div className="col-12 col-sm-10 col-md-8 col-lg-7 col-xl-9 mx-auto mt40 mb40">
              <div className="card card-round ">
               ...
               <Form >
               <textarea  className="card-round col-md-12" 
                name='signHash' 
                value={this.state.signHash}  
                onChange={e=>this.setState({signHash:e.target.value})} ></textarea>
               <Alert variant="danger"  className="mt40">
                ...
                <button onClick={()=>{this.setState({certi:true});const Args = this.state.signHash.split(","); this.setState({Args : Args});}} className="btn btn-primary ">
                     preview
                </button>
                <button onClick={this.handleRegister} className="btn btn-primary btn-xl js-scroll-trigger combtn combtn1 mt20 mb30">
                     Register Certificate
                </button>
            </div>
            {this.state.certi ? <CertificateBox Hash={this.state.Args[0]} address={this.state.Args[2]}/> : null }
